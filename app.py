...
pca = PCA(n_components=512)  # Reduce to 512 dimensions
...
def classify():
    ...
    if uploaded_file.filename != '':
        ...
        features = base_model.predict(np.expand_dims(image, axis=0)).flatten()
        ...
        predicted_class_index_svm = svm_model.predict(reduced_features)[0]
        ...
        if cnn_model:
            ...
            image_for_cnn = cv2.resize(image, (128, 128)) / 255.0
...
def classify_camera():
    ...
    if image_data:
        ...
        if cnn_model:
            ...
            image_for_cnn = image.resize((128, 128))
